...
export const Leaderboard = () => {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          "https://wsh.vistaran.tech/api/leaderboard/winners"
        );
        ...
        const userDetails = response.data.filter((entry) =>
          entry.members.includes(userId)
        );
        if (
          (userDetails.length > 0 && hideUserDetails) ||
          (userDetails.length == 0 && !hideUserDetails)
        ) {
          hideUserPosition();
        }
  ...
  if (isLoading) {
    return (
      <div className=" text-white text-4xl">Leaderboard is updating...</div>
    );
  }
  ...
  return (
    <div>
      <div className="leaderboard-container flex flex-col justify-center relative">
        <div>
          <p className="final-score text-4xl mb-6 mt-6 text-white">
            Congratulations, You finished {selfEntry.maxProgress}
          </p>
        </div>
        ...
        <div className="leaderboard-content flex flex-col justify-center items-center">
          <LeaderTeamItem
            imageURL="rank1.png"
            rank="1"
            teamName={leaderboardEntries[0].teamName}
            score={leaderboardEntries[0].score ?? "0"}
          />
          <LeaderTeamItem
            imageURL="rank2.png"
            rank="2"
            teamName={leaderboardEntries[1].teamName}
            score={leaderboardEntries[1].score ?? "0"}
          />
          <LeaderTeamItem
            imageURL="rank3.png"
            rank="3"
            teamName={leaderboardEntries[2].teamName}
            score={leaderboardEntries[2].score ?? "0"}
          />
          <LeaderTeamItem
            imageURL="rank4.png"
            rank="4"
            teamName={leaderboardEntries[3].teamName}
            score={leaderboardEntries[3].score ?? "0"}
          />
          <LeaderTeamItem
            imageURL="rank5.png"
            rank="5"
            teamName={leaderboardEntries[4].teamName}
            score={leaderboardEntries[4].score ?? "0"}
          />
        </div>
        <div
          className={`self-heading text-xl font-bold text-white mt-2 ${
        ...
        <div
          className={`self-content flex flex-col justify-center items-center ${
            hideUserDetails ? "hidden" : ""
          }`}
        >
          <LeaderTeamItem
            imageURL="self.png"
            rank={selfEntry.maxProgress}
